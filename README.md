...
View для битсета интересен тем, что над ним тоже может иметь смысл проводить некоторые операции, в том числе побитовые. Для создания `view` используется `bitset::subview(from, count)` или конструкторы `view`.
...
- `subview(std::size_t from = 0, std::size_t count = npos)` &mdash; получить view на:
  - `[from, from + count)`, если `from + count <= size()`;
  - `[from, size())`, если `from + count > size()`.
